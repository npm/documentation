...
If the package does not yet exist, staging it publishes a placeholder package that is visible on [npmjs.com](https://www.npmjs.com). The staged version and its contents are not publicly available until the staged package is approved.
