...
If the package does not yet exist, staging it publishes a placeholder package that is publicly available. The staged version and its contents are not available until the staged package is approved.
