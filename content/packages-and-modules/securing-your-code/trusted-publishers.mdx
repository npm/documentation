...
**Note:** Each package can have multiple trusted publishers configured at the same time, so you can publish from different CI/CD providers or workflows for the same package.
...
You can add or remove your trusted publisher configurations at any time through your package settings on [npmjs.com](https://npmjs.com) → Packages → YOUR_PACKAGE → Settings → Trusted publishing. Each package can have multiple trusted publisher connections at the same time, and each connection can be added or deleted independently as needed. To add another provider (for example, publishing from both GitHub Actions and GitLab CI/CD), click the Add trusted publisher button and select the provider.

<Screenshot src="/packages-and-modules/securing-your-code/trusted-publisher-multiple.png" alt="Screenshot of the Add trusted publisher option for configuring multiple trusted publishers" />
...
Trusted publishing currently supports only cloud-hosted runners. Support for self-hosted runners is intended for a future release. Each package can have multiple trusted publishers configured at the same time. Existing trusted publisher connections cannot be changed. To change them, delete the connection and create a new one.
