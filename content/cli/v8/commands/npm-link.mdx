...
The package name can be optionally prefixed with a scope. See [`scope`](/cli/v8/using-npm/scope). The scope must be preceded by an @-symbol and followed by a slash.
...
If your linked package is scoped (see [`scope`](/cli/v8/using-npm/scope)) your link command must include that scope, e.g.
