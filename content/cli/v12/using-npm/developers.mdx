...
redirect_from:
  ...
  - /cli-documentation/v12/misc/developers
  - /cli-documentation/v12/using-npm/developers
  - /cli/misc/developers
  - /cli/using-npm/developers
  - /cli/v12/misc/developers
  - /misc/developers
...
See [`package.json`](/cli/v12/configuring-npm/package-json) for details about what goes in that file. At the very least, you need:
...
- scripts: If you have a special compilation or installation script, then you should put it in the `scripts` object. You should definitely have at least a basic smoke-test command as the "scripts.test" field. See [scripts](/cli/v12/using-npm/scripts).
...
You can use `npm init` in the root of your package in order to get you started with a pretty basic package.json file. See [`npm init`](/cli/v12/commands/npm-init) for more info.
...
More info at [`npm link`](/cli/v12/commands/npm-link).
...
We recommend developing your package under [`install-strategy=linked`](/cli/v12/using-npm/config#install-strategy). The isolated layout only exposes a package's _declared_ dependencies, so an `import` of an undeclared package fails for you during development instead of resolving by accident, shipping broken, and failing for your users:
