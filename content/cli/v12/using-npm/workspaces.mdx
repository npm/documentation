...
redirect_from:
  ...
  - /cli-documentation/v12/misc/workspaces
  - /cli-documentation/v12/using-npm/workspaces
  - /cli/misc/workspaces
  - /cli/using-npm/workspaces
  - /cli/v12/misc/workspaces
  - /misc/workspaces
...
We also refer to these packages being auto-symlinked during `npm install` as a single **workspace**, meaning it's a nested package within the current local file system that is explicitly defined in the [`package.json`](/cli/v12/configuring-npm/package-json#workspaces) `workspaces` configuration.

### Defining workspaces

Workspaces are usually defined via the `workspaces` property of the [`package.json`](/cli/v12/configuring-npm/package-json#workspaces) file, e.g:
...
You may automate the required steps to define a new workspace using [npm init](/cli/v12/commands/npm-init). For example in a project that already has a `package.json` defined you can run:
...
It's possible to directly add/remove/update dependencies of your workspaces using the [`workspace` config](/cli/v12/using-npm/config#workspace).
...
This demonstrates how the nature of `node_modules` resolution allows for **workspaces** to enable a portable workflow for requiring each **workspace** in such a way that is also easy to [publish](/cli/v12/commands/npm-publish) these nested workspaces to be consumed elsewhere.
