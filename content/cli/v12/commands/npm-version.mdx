...
redirect_from:
  ...
  - /cli-documentation/v12/cli-commands/npm-version
  - /cli-documentation/v12/cli-commands/version
  - /cli-documentation/v12/commands/npm-version
  - /cli-documentation/v12/commands/version
  - /cli-documentation/v12/npm-version
  - /cli-documentation/v12/version
  ...
  - /cli/commands/version
  - /cli/npm-version
  - /cli/v12/cli-commands/npm-version
  - /cli/v12/cli-commands/version
  - /cli/v12/commands/version
  - /cli/v12/npm-version
  - /cli/v12/version
  - /cli/version
...
If supplied with `-m` or [`--message` config](/cli/v12/using-npm/config#message) option, npm will use it as a commit message when creating a version commit. If the `message` config contains `%s` then that will be replaced with the resulting version number. For example:

```bash
npm version patch -m "Upgrade to %s for reasons"
```

If the [`sign-git-tag` config](/cli/v12/using-npm/config#sign-git-tag) is set, then the tag will be signed using the `-s` flag to git. Note that you must have a default GPG key set up in your git config for this to work properly. For example:
...
For the `preversion`, `version` and `postversion` scripts, npm also sets the [environment variables](/cli/v12/using-npm/scripts#environment) `npm_old_version` and `npm_new_version`.
