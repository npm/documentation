...
redirect_from:
  ...
  - /cli-documentation/files/package.json
  - /cli-documentation/v12/configuring-npm/package-json
  - /cli-documentation/v12/configuring-npm/package.json
  - /cli-documentation/v12/files/package-json
  - /cli-documentation/v12/files/package.json
  - /cli/configuring-npm/package-json
  - /cli/configuring-npm/package.json
  - /cli/files/package-json
  - /cli/files/package.json
  - /cli/v12/configuring-npm/package.json
  - /cli/v12/files/package-json
  - /cli/v12/files/package.json
...
A lot of the behavior described in this document is affected by the config settings described in [`config`](/cli/v12/using-npm/config).
...
A name can be optionally prefixed by a scope, e.g. `@npm/example`. See [`scope`](/cli/v12/using-npm/scope) for more detail.
...
See [folders](/cli/v12/configuring-npm/folders#executables) for more info on executables.
...
If only a single file is provided, then it corresponds to `man <pkgname>`, regardless of its actual filename. For example:
...
Man files must end with a number, and optionally a `.gz` suffix if they are compressed. The number dictates which man section the file belongs to.
...
A folder that is full of man pages. Sugar to generate a "man" array by walking the folder. See the note on [`man`](#man) above: as of npm v12, these are no longer installed into the system `man` path.
...
See [`scripts`](/cli/v12/using-npm/scripts) to find out more about writing package scripts.
...
- `tag` A specific version tagged and published as `tag` See [`npm dist-tag`](/cli/v12/commands/npm-dist-tag)
- `path/path/path` See [Local Paths](#local-paths) below
- `npm:@scope/pkg@version` Custom alias for a package See [`package-spec`](/cli/v12/using-npm/package-spec#aliases)
...
These things will be installed when doing `npm link` or `npm install` from the root of a package, and can be managed like any other npm configuration param. See [`config`](/cli/v12/using-npm/config) for more on the topic.
...
Overrides are only considered in the root `package.json` file for a project. Overrides in installed dependencies (including [workspaces](/cli/v12/using-npm/workspaces)) are not considered in dependency tree resolution. Published packages may dictate their resolutions by pinning dependencies or using [`bundleDependencies`](#bundledependencies).
...
Unless the user has set the [`engine-strict` config](/cli/v12/using-npm/config#engine-strict) flag, this field is advisory only and will only produce warnings when your package is installed as a dependency.
...
See [`config`](/cli/v12/using-npm/config) to see the list of config options that can be overridden.

### workspaces

The optional `workspaces` field is an array of file patterns that describes locations within the local file system that the install client should look up to find each [workspace](/cli/v12/using-npm/workspaces) that needs to be symlinked to the top level `node_modules` folder.
...
See [`workspaces`](/cli/v12/using-npm/workspaces) for more examples.
...
- [npm init](/cli/v12/commands/npm-init)
- [npm version](/cli/v12/commands/npm-version)
- [npm config](/cli/v12/commands/npm-config)
- [npm help](/cli/v12/commands/npm-help)
- [npm install](/cli/v12/commands/npm-install)
- [npm publish](/cli/v12/commands/npm-publish)
