...
redirect_from:
  ...
  - /cli-documentation/v12/configuring-npm/npmrc
  - /cli-documentation/v12/files/npmrc
...
For a list of available configuration options, see [config](/cli/v12/using-npm/config).
...
npm only recognizes its own [configuration options](/cli/v12/using-npm/config). If your `.npmrc` contains keys that are not part of npm's config definitions (for example, `electron_mirror` or `sass_binary_site`), npm will emit a warning:
