...
github_path: docs/lib/content/configuring-npm/npm-extension.md
redirect_from:
  - /cli-documentation/configuring-npm/npm-extension
  - /cli-documentation/files/npm-extension
  - /cli-documentation/v12/configuring-npm/npm-extension
  - /cli-documentation/v12/files/npm-extension
  - /cli/configuring-npm/npm-extension
  - /cli/files/npm-extension
  - /cli/v12/files/npm-extension
...
`.npm-extension` is the imperative counterpart to the declarative [`packageExtensions`](/cli/v12/configuring-npm/package-json#packageextensions) field, and runs in the same pre-resolution phase, **before** `packageExtensions`. Prefer `packageExtensions` for simple, data-only repairs; reach for `.npm-extension` when you need comments and links explaining a repair, conditional logic, repeated repairs expressed as code, deletion or range rewrites, stale-repair guards, or a policy location outside `package.json`.
...
The [`extension-file`](/cli/v12/using-npm/config#extension-file) config selects a different project-local file. It must resolve inside the project root and use a `.mjs` or `.cjs` extension, and it is honored only from project config or the command line — never from user, global, or builtin config.
...
- [package.json `packageExtensions`](/cli/v12/configuring-npm/package-json#packageextensions)
- [package-lock.json](/cli/v12/configuring-npm/package-lock-json)
- [config](/cli/v12/using-npm/config)
