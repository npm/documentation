...
- `npm install` (in package directory, no arguments):
  ...
  By default, `npm install` will install all modules listed as dependencies in [`package.json`](/cli/v6/configuring-npm/package-json).
...
- `npm install [<@scope>/]<name>`:
  ...
  Do a `<name>@<tag>` install, where `<tag>` is the "tag" config. (See [`config`](/cli/v6/using-npm/config). The config's default value is `latest`.)
...
- `npm install <alias>@npm:<name>`:
  ...
  `<scope>` is optional. The package will be downloaded from the registry associated with the specified scope. If no registry is associated with the given scope the default registry is assumed. See [`scope`](/cli/v6/using-npm/scope).
...
- `npm install [<@scope>/]<name>@<version range>`:
  ...
  Install a version of the package matching the specified version range. This will follow the same rules for resolving dependencies described in [`package.json`](/cli/v6/configuring-npm/package-json).
...
The `-g` or `--global` argument will cause npm to install the package globally rather than locally. See [folders](/cli/v6/configuring-npm/folders).
...
The `--ignore-scripts` argument will cause npm to not execute any scripts defined in the package.json. See [`scripts`](/cli/v6/using-npm/scripts).
...
The `--no-audit` argument can be used to disable sending of audit reports to the configured registries. See [`npm-audit`](npm-audit) for details on what is sent.

See [`config`](/cli/v6/using-npm/config). Many of the configuration params have some effect on installation, since that's most of what npm does.
