...
The token list displays information about each token, including:

- Token name and value (partially obscured)
- Whether the token is configured to bypass two-factor authentication (shows a checkmark for granular access tokens with bypass enabled)
- Creation date, last used date, and expiration date

To view detailed information about a granular access token, including its full permissions, allowed IP ranges, and bypass 2FA configuration, click on the token name.
